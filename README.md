...
Оптимизация выполняется с помощью меметического алгоритма, который сочетает генетический алгоритм с локальным поиском. Поддерживаются два режима: **оптимизация** (максимизация CAI) и **гармонизация** (соответствие распределению частот кодонов хозяина).

## Улучшения в коде (SOTA Level)

### 1. Высокая производительность
- **Параллельные вычисления**: Использование `ProcessPoolExecutor` для одновременной оценки всей популяции. Это значительно ускоряет работу меметического алгоритма на многоядерных процессорах.
- **Многоуровневое кэширование**:
  - Кэширование полной приспособленности последовательностей.
  - Кэширование результатов `RNAfold` по 5'-префиксам (исключает избыточные вызовы внешних программ).
  - Эффективное использование ресурсов при локальном поиске.

### 2. Продвинутые алгоритмы
- **Режим Гармонизации**: Добавлена возможность гармонизации кодонов (Codon Harmonization), что позволяет имитировать естественное распределение кодонов организма-хозяина, предотвращая перегрузку пула тРНК.
- **Умный Локальный Поиск**: Локальный поиск теперь фокусируется на проблемных участках (например, местах со слабым MFE или нежелательными мотивами), а не просто на случайных позициях.
- **Diversity-Aware Selection**: Турнирная селекция учитывает генетическое разнообразие (расстояние Хэмминга), что предотвращает преждевременную сходимость алгоритма.

### 3. Универсальность для разных бактерий
...
- `tests/test_codon_optimizer.py`: Модульные тесты.
- `run.py`: Точка входа для запуска оптимизации.
